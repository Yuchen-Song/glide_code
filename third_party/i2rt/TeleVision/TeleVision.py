...
class OpenTeleVision:
    ...
    async def on_hand_move(self, event, session, fps=60):
        ...
        try:
            ...
            # Update left hand data if present
            if 'left' in hand_data and hand_data['left'] is not None:
                # The first 16 values are the wrist transform (leftHand equivalent)
                left_full_data = np.array(hand_data['left'])
                if len(left_full_data) >= 16:
                    self.left_hand_shared[:] = left_full_data[:16]  # Wrist transform
                    self.left_landmarks_shared[:] = left_full_data  # Full 25 joint data
                    with self._left_hand_valid_flag_shared.get_lock():
                        self._left_hand_valid_flag_shared.value = True
                else:
                    print(f"Warning: Left hand data size is {len(left_full_data)}, expected 16")

            # Update right hand data if present
            if 'right' in hand_data and hand_data['right'] is not None:
                # The first 16 values are the wrist transform (rightHand equivalent)  
                right_full_data = np.array(hand_data['right'])
                if len(right_full_data) >= 16:
                    self.right_hand_shared[:] = right_full_data[:16]  # Wrist transform
                    self.right_landmarks_shared[:] = right_full_data  # Full 25 joint data
                    with self._right_hand_valid_flag_shared.get_lock():
                        self._right_hand_valid_flag_shared.value = True
                else:
                    print(f"Warning: Right hand data size is {len(right_full_data)}, expected 16")
            
            # Update left hand gesture state
            if 'leftState' in hand_data and hand_data['leftState'] is not None:
                state = hand_data['leftState']
                self.left_hand_state_shared[0] = float(state.get('pinch', False))
                self.left_hand_state_shared[1] = float(state.get('squeeze', False))
                self.left_hand_state_shared[2] = float(state.get('tap', False))
                self.left_hand_state_shared[3] = float(state.get('pinchValue', 0.0))
                self.left_hand_state_shared[4] = float(state.get('squeezeValue', 0.0))
                self.left_hand_state_shared[5] = float(state.get('tapValue', 0.0))
            
            # Update right hand gesture state
            if 'rightState' in hand_data and hand_data['rightState'] is not None:
                state = hand_data['rightState']
                self.right_hand_state_shared[0] = float(state.get('pinch', False))
                self.right_hand_state_shared[1] = float(state.get('squeeze', False))
                self.right_hand_state_shared[2] = float(state.get('tap', False))
                self.right_hand_state_shared[3] = float(state.get('pinchValue', 0.0))
                self.right_hand_state_shared[4] = float(state.get('squeezeValue', 0.0))
                self.right_hand_state_shared[5] = float(state.get('tapValue', 0.0))

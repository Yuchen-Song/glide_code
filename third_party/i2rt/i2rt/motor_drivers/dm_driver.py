            ...
            while self.running:
                try:
                    ...
                    with self.command_lock:
                        ...
                        except RuntimeError as comm_err:
                            if "loss communication" in str(comm_err):
                                self._comm_error_count = getattr(self, '_comm_error_count', 0) + 1
                                logging.warning(f"Transient CAN error ({self._comm_error_count}x): {comm_err}")
                                if self._comm_error_count >= 10:
                                    logging.error(f"Too many consecutive CAN errors ({self._comm_error_count}), stopping")
                                    self.running = False
                                    raise
                                time.sleep(0.002)
                                continue
                                ...
                                raise
                        self._comm_error_count = 0  # reset on success
        ...
        for idx, motor_info in enumerate(self.motor_list):
            ...
            for attempt in range(3):
                try:
                    ...
                    last_exc = e
                    if attempt < 2:
                        logging.warning(f"CAN retry {attempt+1}/3 for motor {motor_id} on {self}: {e}")
                        time.sleep(0.001)
            if last_exc is not None:
                logging.error(f"{idx}th motor at DMChainCanInterface {self} failed after 3 retries with info {motor_info}")
                raise last_exc
